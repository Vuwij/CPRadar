...
class PyX4M300(_object):
    ...
    def load_profile(self, profileid):
        ...
        If another profile is loaded, the other profile is unloaded before the presence
        profile is loaded. The profile does not start, the module remains idle. profile
        = 0x014d4ab8 : presence profile  
    ...
    def set_led_control(self, mode, intensity):
        ...
        The setting survives a power cycle. mode = 0 : OFF mode = 1 : simple mode = 2 :
        full (default) Intensity: Future use, 0=low, 100=high  
    ...
    def set_output_control(self, output_feature, output_control):
        """
        set_output_control(PyX4M300 self, uint32_t const output_feature, uint32_t const output_control)
        ...
        ----------
        * `output_feature` :  
            PresenceSingle PresenceMovingList PresenceStaticList BasebandIQ BasebandAP
            PulseDoppler  
        * `output_control` :  
            0 = disable >0 = enable + format. E.g. pulsedoppler output data byte or
            float  
...
_moduleconnectorwrapper.XTS_SDC_STIGS_COMMAND_swigconstant(_moduleconnectorwrapper)
